...
JSON（JavaScript Object Notation）是一种轻量级的数据交换格式，几乎出现在所有现代 Web 开发和 API 设计中：从浏览器与后端通信，到配置文件（如 `package.json`、`tsconfig.json`），再到日志和消息队列。  
...
JSON 只支持有限的几种数据类型，这也是它简单且易解析的原因：
...
JSON 不支持：函数、日期对象、正则、`undefined`、注释等。  
...
**要点**：JSON 只支持对象、数组、字符串、数字、布尔和 `null`。
...
**要点**：字符串中的双引号必须转义为 `\"`，换行等控制字符必须用转义序列表示。
...
很多人会把 JSON 和 JavaScript 对象字面量混淆，它们并不完全相同：
...
- JSON 是现代 Web 开发中最基础也最重要的数据格式之一，理解其严格语法能显著减少接口调试时间。  
- 常见错误多来自“写得像 JavaScript 对象”，而不是“严格按 JSON 规范”：单引号、多余逗号、注释、非法值等。  
