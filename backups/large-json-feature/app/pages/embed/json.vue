...
const parsedData = computed(() => {
  // Large files are rendered via the worker-backed lazy tree, never as a
  // materialized object on the main thread.
...
async function formatJsonAsync(silent: boolean) {
  const isHuge = isLargeFile(inputJson.value)

  if (isHuge) {
    lazyTree.reset()
    const streamResult = await parseStream(inputJson.value)
    ...
    if (streamResult.error) {
      ...
      if (repaired) {
        inputJson.value = repaired
        lazyTree.reset()
        const retry = await parseStream(repaired)
        if (!retry.error) {
          lazyTree.initFromSummary(retry.rootType, retry.rootChildCount, retry.nodeCount)
          outputJson.value = ''
          error.value = ''
          parseError.value = null
    ...
    lazyTree.initFromSummary(streamResult.rootType, streamResult.rootChildCount, streamResult.nodeCount)
    outputJson.value = ''
    error.value = ''
    parseError.value = null
    if (!silent) toast.success(t('toast.formatted'))
