...
type Request = LoadRequest | FieldsRequest | ExportRequest | SearchRequest | ResetRequest
...
/** Offsets + lengths of each non-empty line (NDJSON). */
...
self.onmessage = (e: MessageEvent<Request>) => {
  ...
  if (data.mode === 'load') {
    ...
    try {
      ...
      if (docFormat === 'ndjson') {
        lineOffsets = buildLineOffsets(docText)
        // validate every line
        ...
        if (bad) {
          const res: ScanResult = {
            ...
            rootType, format: docFormat, recordCount, candidates, bytes: docText.length,
          }
