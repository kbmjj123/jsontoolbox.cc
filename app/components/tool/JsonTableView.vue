  ...
  <div ref="containerRef" class="h-full overflow-auto rounded-xl border border-surface-200 bg-white dark:border-surface-700 dark:bg-surface-900 relative">
    ...
    <Teleport to="body">
      ...
      >
        ...
        <pre class="text-xs font-mono text-surface-700 dark:text-surface-300 whitespace-pre-wrap break-all leading-relaxed">{{ popover.formatted }}</pre>
      </div>
    </Teleport>
  </div>
</template>
...
function showPopover(rowIndex: number, col: string, event: MouseEvent) {
  ...
  popover.col = col
  popover.formatted = JSON.stringify(value, null, 2)
