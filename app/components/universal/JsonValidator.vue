  ...
  <ResizablePanel v-model:fullscreen="fullscreen" :initial-ratio="0.5" responsive>
    <template #first>
      <div class="h-full pr-3">
        ...
        >
          <template #actions>
            <button v-if="inputJson" @click="copyInput" class="text-xs text-primary-600 hover:text-primary-700 dark:text-primary-400">
    ...
    <template #second>
      <div class="h-full pl-3">
        ...
        <div class="h-full overflow-auto">
            ...
            <div v-if="result !== null">
              ...
              <div v-if="result.valid" class="flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20">
                ...
                <div>
                  ...
                  <p class="text-xs text-green-600 dark:text-green-500 mt-0.5">{{ tool.ui?.hint_valid || 'Your JSON is syntactically correct.' }}</p>
                </div>
              </div>
              ...
              <div v-else class="rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20">
                ...
                <button v-if="result.error" @click="copyError" class="mt-2 text-xs text-primary-600 hover:text-primary-700 dark:text-primary-400">
          ...
          <div v-if="result === null" class="flex h-full items-center justify-center text-surface-400 dark:text-surface-500 text-sm">
    ...
    <template #toolbar-left>
      ...
      <button v-if="result && !result.valid" @click="fixJson" class="rounded-xl border border-surface-200 bg-white px-4 py-2 text-xs font-bold text-surface-700 hover:bg-surface-50 dark:border-surface-700 dark:bg-surface-800 dark:text-surface-300">
...
const copyError = async () => {
  if (result.value?.error) {
    await copyToClipboard(`Error: ${result.value.error}\nLine: ${result.value.line}, Column: ${result.value.column}`)
    copyErrorCopied.value = true
    setTimeout(() => { copyErrorCopied.value = false }, 2000)
  }
}
</script>
