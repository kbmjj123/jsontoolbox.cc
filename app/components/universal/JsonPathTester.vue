<template>
  <ResizablePanel v-model:fullscreen="fullscreen" :initial-ratio="0.4" responsive>
      ...
      <div class="h-full pl-3 overflow-auto">
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <span v-if="results.length > 0" class="text-xs text-surface-500 dark:text-surface-400">{{ results.length }} match(es)</span>
          </div>
    ...
    <template #toolbar-left>
      <button @click="loadExample" class="text-xs text-primary-600 hover:text-primary-700 dark:text-primary-400">Example</button>
  ...
  <div class="mt-4 rounded-xl border border-surface-200 dark:border-surface-700 overflow-hidden">
    <button @click="showReference = !showReference" class="w-full flex items-center justify-between px-4 py-3 bg-surface-50 dark:bg-surface-800 hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors">
      <span class="text-sm font-bold text-surface-700 dark:text-surface-300"><Icon name="lucide:book-open" class="h-4 w-4 mr-2 inline" />JSONPath Reference</span>
      ...
      <div class="mb-4">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-xs">
            <thead><tr class="border-b border-surface-200 dark:border-surface-700"><th class="text-left py-1 px-2 font-bold text-surface-600 dark:text-surface-400">Operator</th><th class="text-left py-1 px-2 font-bold text-surface-600 dark:text-surface-400">Description</th><th class="text-left py-1 px-2 font-bold text-surface-600 dark:text-surface-400">Example</th></tr></thead>
...
const commonPaths = ['$', '$.*', '$[0]', '$[*]', '$.store', '$.store.books[*]', '$.store.books[0]', '$.store.books[*].title', '$.store.books[?(@.price < 10)]']
...
const getByPath = (obj: any, path: string): any[] => {
  ...
  const traverse = (current: any, currentPath: string, partIndex: number) => {
    if (partIndex >= parts.length) { results.push({ path: currentPath, value: current }); return }
    const part = parts[partIndex]
    if (current === null || current === undefined) return
    if (Array.isArray(current)) {
      if (part === '*') current.forEach((item, i) => traverse(item, `${currentPath}[${i}]`, partIndex + 1))
      else if (/^\d+$/.test(part)) { const index = parseInt(part); if (index < current.length) traverse(current[index], `${currentPath}[${index}]`, partIndex + 1) }
    } else if (typeof current === 'object') {
      if (part === '*') Object.keys(current).forEach(key => traverse(current[key], `${currentPath}.${key}`, partIndex + 1))
      else if (part in current) traverse(current[part], `${currentPath}.${part}`, partIndex + 1)
    }
  }
